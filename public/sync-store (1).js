...
// mode: 'today' | -1 | -2 | ... (负数=往前N天)
function trainModeToDate(mode) {
  const d = new Date();
  const m = mode ?? window.S_trainDate;
  if (!m || m === 'today') return d;
  const offset = typeof m === 'number' ? m : (m === 'yesterday' ? -1 : parseInt(m) || 0);
...
function getTrainLogTimestamp() {
  if (isTrainToday()) return new Date().toISOString();
  const d = trainModeToDate();
  const now = new Date();
...
async function loadTrainDraftForMode(mode) {
  ...
  if (await loadTrainFromCloudForDay(mode)) {
    ...
    if (!isTrainToday() && typeof toast === 'function') {
      const { label } = trainDayRange(mode);
      const dow = ['日','一','二','三','四','五','六'][label.getDay()];
      toast(`已从云端载入${label.getMonth()+1}/${label.getDate()}（周${dow}）训练，可继续补全`);
    }
    return;
  }
  if (isTrainToday()) {
    // 今日模式才清空，补录模式只清空workout不影响今日数据
    S.workout = [];
    S.volume = 0;
    ...
    // 补录模式：清空编辑区但保持今日数据在内存里
    S.workout = [];
    S.volume = 0;
    S.trainCloudSessionId = null;
    if (typeof renderExercises === 'function') {
      const list = document.getElementById('ex-list');
      if (list) list.innerHTML = '<div class="empty">选择肌群或生成方案开始补录</div>';
    }
  }
...
function updateTrainAutosaveHint(state) {
  ...
  const map = {
    ...
    cloud: '已同步云端（可随时补全）',
    syncing: '正在同步云端…',
    restored: '已恢复未完成的训练草稿 · 继续填写即可',
    empty: isTrainToday() ? '填写动作、重量后会自动保存草稿' : '补录模式：填写完成后点「记录」保存到该日',
    err: '本机已保存，云端同步失败',
  };
